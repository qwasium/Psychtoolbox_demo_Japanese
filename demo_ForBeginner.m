...
% 特に120Hzディスプレイ、4Kディスプレイなどの高性能なディスプレイを使う際は接続規格やハ
% ードウェアの適合、プログラムの内容を十分に点検すること。
% わからない人は自分で判断せずにわかる人に聞くこと。
% 
% ・ディスプレイケーブルについて
% 最低限でも以下の4種類のディスプレイケーブルの種類を知っておくこと。
% 常識レベルの知識なので知らない人は今すぐググること。
% ・HDMI          ：デジタル、音声あり、初期世代は適合に注意
% ・Display Port  ：デジタル、音声あり、基本これを使え
% ・DVI           ：デジタル及びアナログ、音声なし、4K非対応
% ・VGA           ：アナログ、音声なし、フルHD非対応、初心者は使用禁止
% 映像通信規格について勉強したくない人は以下のルールに従うこと。規格に精通している人は
% 各自好きに判断して使ってもらって構わない。
% 【重要】
% ・基本はDisplay Portを使うこと、DPが無い場合はHDMIを使うこと
% ・変換アダプタ・変換ケーブルは一切使用禁止
% ・実験に関わるときは刺さればいいや、映ればいいやの発想は捨てること
% アダプタについては規格に精通している人はパッシブアダプタは使用しても良いが、判断できな
% い人は決して手を出すべきではない。
% (例：DVI-HDMI変換アダプタ、ただしフルHDを超えない範囲での使用かつDVIデジタルとHDMI
% のパッシブ変換のものに限る)
% 
% ・ストレージについて
...
% これを堂々と使っている人がなんと多いことか。
% なぜこれがマズいのかわからない人はPTBをなぜ使っているのか考え直してほしい。
% ※PTB開発者の実際の発言："私は世界一アホな研究者です！と叫ぶ行為に等しい。"
% 開発者ですら辛辣な発言をしているので、SyncErrorが出たらまずは環境を見直すこと。
% 
...
% 処理を強制終了するようにできる。
    ...
    % いる。
    % 
    % ジャギー対策でアンチエイリアシングを使う場合は以下のコマンドでウィンドウを開く。
